...
module.exports = class MessageEvent extends AbstractEvent {
  ...
  async run(msg) {
    ...
    if (!msg.guild.shard) return msg.channel.send(`An error has occured, Try something like kicking the bot and inviting it again, if it doesn't solve your issue, join our support server if it doesn't resolve ${this.client.config.supportServer("A3KNganCsK")}`);
    /*
    if (command.voteLock) {
